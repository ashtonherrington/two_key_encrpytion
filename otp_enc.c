...
int main(int argc, char *argv[]){
   ...
   char testArray[70000];

   bzero(testArray, 70000);

   fgets(testArray, 70000, test);

   int ts;
   for(ts=0; ts<70000; ts++){
      if(testArray[ts] < 'A'){
         if(testArray[ts] != ' '){
	    if(testArray[ts] != '\n'){
	       if(testArray[ts] != '\0'){
	          fprintf(stderr, "Invalid characters present in %s, exiting...\n\n", argv[1]);
		  //fputs("Invalid characters present, exiting...\n\n", stderr);
	       //fputs("%s, exiting...", argv[1]);
	       exit(1);
	       }
	    }
	 }
      }
      else if(testArray[ts] > 'Z'){
//	 fputs("Invalid characters in file , exiting...\n\n", stderr);
	          fprintf(stderr, "Invalid characters present in %s, exiting...\n\n", argv[1]);
//	 fputs("%s, exiting...", argv[1]);
	 exit(1);	 
      }
